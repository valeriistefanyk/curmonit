...
class GetApiRates(BaseController):
    ...
    def _filter(self, xrates):
        ...
        if "from_currency" in args:
            xrates = xrates.where(ExRate.from_currency == args.get("from_currency"))
        if "to_currency" in args:
            xrates = xrates.where(ExRate.to_currency == args.get("to_currency"))
        return xrates
...
class UpdateRates(BaseController):
    def _call(self, from_currency = None, to_currency = None):
    ...
    def _update_rate(self, from_currency, to_currency):
        api.update_xrate(from_currency, to_currency)
...
class EditRate(BaseController):
    def _call(self, from_currency, to_currency):
        ...
        upd_count = (ExRate.update({ExRate.rate: float(request.form["new_rate"]), ExRate.updated: datetime.datetime.now()})
                    .where(ExRate.from_currency == from_currency, to_currency == to_currency).execute())
